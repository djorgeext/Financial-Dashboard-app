...
FRONTEND_TEMPLATE_PATH = FRONTEND_DIR / "templates" / "dashboard.html"
LEGACY_TEMPLATE_PATH = BASE_DIR / "dashboard.html"
FRONTEND_STATIC_DIR = FRONTEND_DIR / "static"
LEGACY_STATIC_DIR = BASE_DIR / "static"


def _resolve_dashboard_template_path() -> Path:
    """Resolve dashboard template path with backward-compatible fallback."""
    if FRONTEND_TEMPLATE_PATH.exists():
        return FRONTEND_TEMPLATE_PATH
    return LEGACY_TEMPLATE_PATH


def _resolve_static_dir() -> Path:
    """Resolve static directory path with backward-compatible fallback."""
    if FRONTEND_STATIC_DIR.exists():
        return FRONTEND_STATIC_DIR
    if LEGACY_STATIC_DIR.exists():
        return LEGACY_STATIC_DIR
    raise FileNotFoundError(
        f"Static directory not found. Checked: {FRONTEND_STATIC_DIR} and {LEGACY_STATIC_DIR}"
    )
...
async def get_forecast(
    sector: str,
    granularity: str = FastApiPath(..., pattern="^(daily|hourly)$")
) -> Dict:
    """
...
async def serve_root() -> HTMLResponse:
    ...
    return FileResponse(_resolve_dashboard_template_path(), media_type="text/html")
...
def main() -> None:
    ...
    logger.info(f"Starting server on {runtime_config.HOST}:{runtime_config.PORT}")
    app_target = "backend.app:app" if runtime_config.DEBUG else app
    uvicorn.run(
        app_target,
        host=runtime_config.HOST,
        port=runtime_config.PORT,
        log_level="info",
        reload=runtime_config.DEBUG,
    )
